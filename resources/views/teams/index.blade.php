...
<div>
    <div class="panelHeader">
        <h3 class="panelHeaderTitle">Team</h3>
        @if(canAccess(['team create']))
        <a href="{{ route('admin.teams.create') }}" class="panelHeaderBtn">
        ...
        <table id="dataTable">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <th>
                        @if (!empty($row->image))
                        <img src="{{ !empty($row->image) ? asset($row->image) : asset('/images/placeholder.webp') }}"
                            width="50px" height="50px" alt="Featured Image">
                        @else
                    ...
                    <td>
                        <div class="flex items-center justify-end gap-1">
                            @if(canAccess(['team edit']))
                            <a href="{{ route('admin.teams.edit', $row->id) }}" class="edit-action-btn">
                                <i class="fa-regular fa-pen-to-square"></i>
                            </a>
                            @endif

                            @if(canAccess(['team destroy']))
                            <a href="{{ route('admin.teams.destroy', $row->id) }}"
