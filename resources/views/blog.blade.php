    ...
    <div class="container">
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-[30px]">
            ...
            <div>
                ...
                <a href="{{ url('blog-details/' . $row->page_url) }}"
                    class="aspect-[370/251] block rounded-[20px] overflow-hidden">
                    <img class="w-full h-full object-cover" src="{{ asset($row->featured_image) }}" width="370"
                        height="251" alt="{{ $row->title }}">
                </a>
                @endif

                <div class="text-sm flex gap-5 mt-3 lg:mt-6">
                    <span class="flex items-center gap-2">
                        ...
                        <span>{{ !empty($row->created) ? date('M d, Y', strtotime($row->created)) : date('M d, Y')
                            }}</span>
                    </span>
                    ...
                    <span class="flex items-center gap-2">
                        ...
                        <span>{{ (!empty($row->read_time) ? $row->read_time : 0) }} min read</span>
                    </span>
                </div>
                ...
                <div class="flex items-center gap-[10px] text-sm">
                    @if(!empty($row->userList->avatar))
                    <img src="{{ asset($row->userList->avatar) }}" alt="{{ $row->userList->name }}" width="35"
                        height="35" class="rounded-full">
                    @endif
                    <span class="text-[#212121] text-sm font-semibold leading-[22px]">{{ (!empty($row->userList->name) ?
                        $row->userList->name : 'N/A') }}</span>
                </div>
            </div>
            @endforeach
            @endif
