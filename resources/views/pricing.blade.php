...
<div class="bg-[#FAF9F9]">
    <div class="container">
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8 xl:gap-12   py-24">
            @foreach ($info->packages as $key => $row)
            <div
                class="{{ $row->is_recommended == 1 ? 'bg-[#0B121F] hover:' : 'bg-[#FFF]' }} border border-1 border-[#E0E0E0]  rounded-[30px] pt-[30px] pb-[42px] px-6 lg:px-4 xl:px-[30px]  hover:translate-y-[-10px] transition duration-700 group">
                <span
                ...
                <div class="relative pt-[72px] space-y-3">
                    @if ($row->is_recommended == 1)
                    <div class="absolute top-[33px]">
                        <span
                    ...
                    <p class="text-[#908E8E] text-[14px] font-medium">
                        {{ $row->title }}
                    </p>
                </div>

                <a href="{{ route('user.order.checkout', $row->id) }}"
                    class="pricing-btn {{ $row->is_recommended == 1 ? 'bg-primary text-black group-hover:bg-secondary group-hover:text-white group-hover:border-secondary ' : 'bg-[#FFF] group-hover:bg-primary' }} ">
                    Choose Plan
                </a>
