...
<div class="border border-[#E8E8E8] rounded-[4px] py-6 px-[30px]">
    ...
    <form method="POST" action="{{ route('publisher.profile.update') }}" enctype="multipart/form-data">
        @csrf
        <h5 class="px-[30px] py-2 rounded-[4px] bg-[#F6F6F6] text-[#2B2B2B] font-semibold uppercase mb-5 text-xl">UPDATE
            YOUR ACCOUNT DETAILS</h5>
    ...
    <!-- Update Password -->
    <form method="POST" action="{{ route('publisher.profile.update-password') }}" class="mt-[60px]">
        @csrf
        <h5 class="px-[30px] py-2 rounded-[4px] bg-[#F6F6F6] text-[#2B2B2B] font-semibold uppercase mb-5 text-xl">UPDATE
            YOUR PASSWORD</h5>
